...
def render_yearly_compare_mode():
    ...
    X: Monitoring（月日を ANCHOR_YEAR に置換）
    Y: 同一 Area×Drop 内の累積付着（cumsum）
    サイズ: 殻長平均（mm）
    塗り: 大型幼生（>=250µm）区間累積（Drop→Monitoring、当日除外）
    枠: 投入年（凡例）
    """
    import streamlit as st
    import pandas as pd
    ...
    c1, c2, c3 = st.columns([1.3, 1.8, 1.7])
    with c1:
        area_sel = st.selectbox('エリア', ['ALL'] + areas, index=0, key='yc_area')
    with c2:
        scallop_is_cumulative = st.checkbox('Scallopは既に累積値（cumsumしない）', value=False, key='yc_scallop_cum')
    with c3:
        use_log_color = st.checkbox('幼生累積（色）をlog表示', value=True, key='yc_log_color')
    ...
    df_e['duration_days'] = (pd.to_datetime(df_e['Monitoring_Date']) - pd.to_datetime(df_e['Drop_Date'])).dt.days
    df_e['duration_days'] = pd.to_numeric(df_e['duration_days'], errors='coerce').fillna(0).astype(int)

    def _calc_x(area: str, drop_day, mon_day):
        larv = larv_by_area.get(area)
        if larv is None or larv.empty:
            return 0.0, 0
        drop = pd.to_datetime(drop_day).date()
        mon = pd.to_datetime(mon_day).date()
        msk = (larv['date'] >= drop) & (larv['date'] < mon)
        return float(larv.loc[msk,'X_large_day'].sum()), int(msk.sum())

    tmp = df_e.apply(lambda r: _calc_x(r['Area'], r['Drop_Date'], r['Monitoring_Date']), axis=1, result_type='expand')
    ...
    # shell columns always exist
    for col in ['shell_mean','shell_n']:
        if col not in df_plot.columns:
            df_plot[col] = np.nan

    # Shell match (group-wise asof)
    SHELL_TOL_DAYS = 5
    if df_s is not None and (not df_s.empty):
        ...
        if shell_col is not None:
            df_s[shell_col] = pd.to_numeric(df_s[shell_col], errors='coerce')
            shell_daily = (
            ...
            df_plot = df_plot.assign(
                Drop_Date_dt=pd.to_datetime(df_plot['Drop_Date']).dt.floor('D'),
                Monitoring_Date_dt=pd.to_datetime(df_plot['Monitoring_Date']).dt.floor('D'),
            )
    ...
    # shell fill
    df_plot['shell_filled'] = pd.to_numeric(df_plot.get('shell_mean'), errors='coerce')
    for (a, d), g in df_plot.groupby(['Area','Drop_Date'], sort=False):
        ...
        filled.loc[msk & prev.notna() & nxt.isna()] = prev.loc[msk & prev.notna() & nxt.isna()]
        filled.loc[msk & prev.isna() & nxt.notna()] = nxt.loc[msk & prev.isna() & nxt.notna()]
        df_plot.loc[g.index, 'shell_filled'] = filled.values

    # marker size
    size_min, size_max = 6.0, 40.0
    vv = pd.to_numeric(df_plot['shell_filled'], errors='coerce')
    if vv.notna().any():
        vmin, vmax = float(vv.min()), float(vv.max())
    else:
        vmin, vmax = np.nan, np.nan
    if pd.notna(vmin) and pd.notna(vmax) and vmax > vmin:
        df_plot['marker_size'] = size_min + (vv - vmin) / (vmax - vmin) * (size_max - size_min)
    else:
    ...
    if scallop_is_cumulative:
        df_plot['Y_cum'] = df_plot['Y_inc']
    else:
        df_plot['Y_cum'] = df_plot.groupby(['Area','Drop_Date'], sort=False)['Y_inc'].cumsum()

    # X seasonal axis
    df_plot['Mon_md'] = pd.to_datetime(df_plot['Monitoring_Date'], errors='coerce').dt.strftime('%m-%d')
    df_plot['Mon_anchor'] = pd.to_datetime(df_plot['Mon_md'].apply(lambda s: f"{ANCHOR_YEAR}-{s}"), errors='coerce')

    df_plot = df_plot.dropna(subset=['Mon_anchor','Y_cum']).copy()
    if df_plot.empty:
        st.warning('表示できる点がありません（Monitoring_Date / 付着数が欠損している可能性があります）')
        return

    # fill color (larvae)
    x_raw = pd.to_numeric(df_plot['X_total'], errors='coerce').fillna(0.0)
    if use_log_color:
        df_plot['X_color'] = np.log1p(x_raw)
        cbar_title = '大型幼生累積(≥250) log(1+x)'
    else:
        df_plot['X_color'] = x_raw
        cbar_title = '大型幼生累積(≥250)'

    # outline color by year
    ...
    outline_colors = [year_to_color.get(int(y), '#333') for y in df_plot['Drop_Year'].fillna(-1).astype(int).tolist()]

    # line id for series
    df_plot['Drop_md'] = pd.to_datetime(df_plot['Drop_Date']).dt.strftime('%m/%d')
    ...
    for _, g in df_plot.groupby('line_id', sort=False):
        g = g.sort_values('Monitoring_Date')
        fig.add_trace(go.Scatter(
            x=g['Mon_anchor'], y=g['Y_cum'], mode='lines',
            line=dict(color='rgba(0,0,0,0.25)', width=1.2, dash='dot'),
            showlegend=False, hoverinfo='skip'
        ))

    fig.add_trace(go.Scatter(
        x=df_plot['Mon_anchor'], y=df_plot['Y_cum'], mode='markers',
        marker=dict(
            size=df_plot['marker_size'],
            color=df_plot['X_color'],
            colorscale='Viridis',
            showscale=True,
            colorbar=dict(title=cbar_title),
            opacity=0.88,
            line=dict(color=outline_colors, width=2.0),
        ),
        customdata=np.c_[
            df_plot['Area'].astype(str).values,
            df_plot['Drop_md'].astype(str).values,
            df_plot['Mon_md'].astype(str).values,
            df_plot['Drop_Year'].fillna(-1).astype(int).values,
            df_plot['duration_days'].astype(int).values,
            df_plot['overlap_days'].astype(int).values,
            df_plot['X_total'].round(2).values,
            df_plot['Y_inc'].round(2).values,
            df_plot['Y_cum'].round(2).values,
            pd.to_numeric(df_plot['shell_mean'], errors='coerce').round(2).values,
            pd.to_numeric(df_plot['shell_filled'], errors='coerce').round(2).values,
        ],
        hovertemplate=(
    ...
    fig.update_layout(
        ...
        title=dict(text='季節軸（年無視）：Monitoring日 × 累積付着（サイズ=殻長、塗り=幼生累積、枠=年）', x=0.01, xanchor='left', y=0.98),
        legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='left', x=0.0)
    )
    fig.update_xaxes(title_text='Monitoring（MM-DD）', tickformat='%m-%d', type='date', showgrid=True, gridcolor='rgba(0,0,0,0.06)')
    fig.update_yaxes(title_text='累積付着数（同一Drop内）', showgrid=True, gridcolor='rgba(0,0,0,0.06)')

    st.caption('注：濃い色（幼生累積が多い）のに累積付着が伸びない系列は「幼生は多いが付着しない」逸脱の候補です。')
    st.plotly_chart(fig, use_container_width=True)
