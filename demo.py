            ...
            with st.expander(f"Area: {area}", expanded=False):  # ← タイトルは「Area: xxx」を維持
                ...
                try:
                    ...
                    st.download_button(
                        "このエリアのサマリーをダウンロード",
                        data=csv_bytes,
                        file_name=f"larvae_summary_{area}.csv",
                        mime="text/csv",
        ...
        with st.expander("採苗数データ表（クリックで展開）", expanded=False):
            ...
            try:
                ...
                st.download_button(
                    label="表をCSVでダウンロード",
                    data=csv_bytes,
                    file_name=f"scallop_table_{area}.csv",
                    mime="text/csv",
                    use_container_width=True
                )
            except Exception:
...
def render_map_mode():
    ...
    with st.sidebar:
        mode = st.radio('地図の表示データ', ['GSI', 'ラーバ'], index=0, key='map_mode')
        years_all = sorted(set(df_gsi['Year'].dropna().unique()).union(set(df_larv['Year'].dropna().unique())))
        if not years_all:
            st.info('年度データがありません。'); return
        sel_year = st.selectbox('年度', years_all, index=len(years_all)-1, key='map_year')
        # 週リスト（モードごと）
        ...
        if not weeks_all:
            st.info('選択年度に週データがありません。'); return
        ...
        st.session_state['map_week_idx'] = min(
            max(int(st.session_state['map_week_idx']), 0),
            max(0, n - 1)
        )        

        st.markdown("<div style='text-align:center;font-size:13px;'>週選択</div>", unsafe_allow_html=True)
        ...
        with c_next:
            if st.button('＋', key='map_next'):
                st.session_state['map_week_idx'] = min(max(0, n - 1),
        st.session_state['map_week_idx'] + 1)
        ...
        if n == 0:
            ...
            safe_idx = st.session_state['map_week_idx']
            sel_label = st.selectbox('表示週', options, index=safe_idx, key='map_week_label')

            try:
                sel_week = next(w for w, lab in week_labels if lab == sel_label)
                st.session_state['map_week_idx'] = options.index(sel_label)

            except StopIteration:
                sel_week = weeks_all[-1] if len(weeks_all) > 0 else None
                if sel_week is not None:    
                    try:
                        fallback_label = next(lab for w, lab in week_labels if w == sel_week)
                        st.session_state['map_week_idx'] = options.index(fallback_label)
                        sel_label = fallback_label
                    except StopIteration:
                        st.session_state['map_week_idx'] = max(0, n - 1)
                        sel_label = options[st.session_state['map_week_idx']]
    ...
    if areas_with_data:
        rows = []
        for area in areas_with_data:
            gsi_F_mean, gsi_F_trend, gsi_M_mean, gsi_M_trend = metrics_gsi(df_gsi, area, sel_year, sel_week, eps=0.2)
            larv_ge200, larv_200_259, larv_ge260, diff_prev, diff_prevY = metrics_larvae(df_larv, area, sel_year, sel_week)
            temp_mean_10m, temp_tr_prev, temp_tr_prevY = metrics_temp10m(base_dir, area, sel_year, sel_week, eps_t=0.3)
            rows.append({
                'Area': area,
                'GSI_F_mean': gsi_F_mean,
                'GSI_F_trend': gsi_F_trend,
                'GSI_M_mean': gsi_M_mean,
                'GSI_M_trend': gsi_M_trend,
                'Larvae_ge200_total': larv_ge200,
                'Larvae_200_259': larv_200_259,
                'Larvae_ge260': larv_ge260,
                'Larvae_diff_prev': diff_prev,
                'Larvae_diff_prevY': diff_prevY,
                'Temp10m_mean': temp_mean_10m,
                'Temp10m_prev_trend': temp_tr_prev,
                'Temp10m_prevY_trend': temp_tr_prevY,
                'WeekLabel': sel_label
            })
        df_summary = pd.DataFrame(rows)
        with st.expander('週要約（一覧）', expanded=True):
            st.dataframe(
                df_summary.style.format({
                    'GSI_F_mean': '{:.2f}',
                    'GSI_M_mean': '{:.2f}',
                    'Larvae_ge200_total': '{:.0f}',
                    'Larvae_200_259': '{:.0f}',
                    'Larvae_ge260': '{:.0f}',
                    'Larvae_diff_prev': '{:.0f}',
                    'Larvae_diff_prevY': '{:.0f}',
                    'Temp10m_mean': '{:.1f}'
                }),
                use_container_width=True
            )
            try:
                ...
                st.download_button(
                    '週要約CSVをダウンロード', data=csv_bytes,
                    file_name=f'map_week_summary_{sel_year}_week{sel_week}.csv',
        ...
        # コメント
        with st.expander('週要約（詳細）', expanded=False):
            def bullet(lines):
                if not lines: return ''
                return "\n".join([f"- {l}" for l in lines])
            for area in areas_with_data:
                g_F, tr_F, g_M, tr_M = metrics_gsi(df_gsi, area, sel_year, sel_week, eps=0.2)
                l_ge200, l_200_259, l_ge260, dp, dy = metrics_larvae(df_larv, area, sel_year, sel_week)
                t_mean, t_prev, t_prevY = metrics_temp10m(base_dir, area, sel_year, sel_week, eps_t=0.3)
                lines = [
                    f"GSI（F）平均 {g_F:.2f}（{tr_F}）" if not pd.isna(g_F) else f"GSI（F）平均 NaN（{tr_F}）",
                    f"GSI（M）平均 {g_M:.2f}（{tr_M}）" if not pd.isna(g_M) else f"GSI（M）平均 NaN（{tr_M}）",
                    f"ラーバ ≥200μm 合計 {int(l_ge200)}（内訳 200–259: {int(l_200_259)} / ≥260: {int(l_ge260)}）",
                    f"前週比（≥200）: {'増加' if (not pd.isna(dp) and dp>0) else ('減少' if (not pd.isna(dp) and dp<0) else '変化なし' if not pd.isna(dp) else 'データ不足')}（差 {int(dp) if not pd.isna(dp) else 'NaN'}）",
                    f"前年同期比（≥200）: {'増加' if (not pd.isna(dy) and dy>0) else ('減少' if (not pd.isna(dy) and dy<0) else '変化なし' if not pd.isna(dy) else 'データ不足')}（差 {int(dy) if not pd.isna(dy) else 'NaN'}）",
                    f"水温（10m）平均 {f'{t_mean:.1f}' if not pd.isna(t_mean) else 'NaN'}℃／前週比 {t_prev}／前年同期比 {t_prevY}"
                ]
                st.markdown(f"**{area}**\n\n" + bullet(lines))
...
def reset_sidebar_state_for(prefix_keep: str):
    prefixes = ('map_', 'sc_', 'larv_', 'water_', 'cal_')
...
def main():
    ...
    try:
        mode = st.segmented_control(
            "モード選択",
            options=["カレンダー", "地図", "水温(GSI)", "ラーバ", "採苗数"],
            key="main_mode",
            default="カレンダー"
        )
    except Exception:
