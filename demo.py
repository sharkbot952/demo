...
def render_yearly_compare_mode():
    ...
    def _calc_x(area: str, drop_day, mon_day):
        ...
        msk = (larv['date'] >= drop) & (larv['date'] < mon)  # Monitoring当日は含めない
    ...
    # --- X：年無視（アンカー年）
    df_plot['Mon_anchor'] = pd.to_datetime(df_plot['Monitoring_Date'].dt.strftime(f'{ANCHOR_YEAR}-%m-%d'), errors='coerce')
